...
<img src="/images/acute_angle.png"></p><img src="/images/obtuse_angle.png"></p>
...
&nbsp; isOutlierPushUps, isOutlierSquats, isOutlierPullups 함수는 영상 촬영 중 해당 관절의 각도 변화를 모니터링하다가, 차이가 크게 나면 그 값을 무시해주는 함수입니다. 실시간으로 매 프레임 별로 자세분석 시 관절의 위치를 잘못 찍는 노이즈 값들이 간혹 식별되었습니다. 이 노이즈 값들이 자세 평가에 반영이 되지 않도록 하는 함수입니다. 매 frame별로 관찰을 하다가 점을 잘못찍었다고 판단이 되면(관찰하고자하는 관절의 각도의 변화량이 급격하면) 해당 프레임을 무시합니다.
